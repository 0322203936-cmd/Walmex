...
Reporte ejecutivo estilo Walmart — lee Excel local
"""
import json, base64, openpyxl, io
...
def cargar_datos() -> dict:
    # Buscar el Excel en varias ubicaciones
    paths = [
        "Analisis_Walmart.xlsx",
        "Analisis Walmart.xlsx",
        "/mnt/user-data/uploads/Analisis_Walmart.xlsx",
    ]
    wb = None
    for p in paths:
        if Path(p).exists():
            ...
            break
    if not wb:
        raise FileNotFoundError("No se encontró Analisis_Walmart.xlsx. Súbelo al repo.")
...
function init(){
  window.onerror=function(m,s,l){document.body.innerHTML='<p style="padding:20px;color:red">Error: '+m+' (línea '+l+')</p>';};
...
function updateHeader(){
  var now=new Date();
  var fecha=DIAS[now.getDay()].charAt(0).toUpperCase()+DIAS[now.getDay()].slice(1)+', '+now.getDate()+' de '+MESES[now.getMonth()]+' de '+now.getFullYear();
  document.getElementById('hdrFecha').textContent=fecha;
  var yr=Math.floor(state.semana/100),wk=state.semana%100;
  document.getElementById('hdrSem').textContent=yr+' · Semana '+String(wk).padStart(2,'0');
  document.getElementById('hdrTienda').textContent=state.tienda;
  document.getElementById('projTitle').textContent='Proyección Semana '+(wk+1);
}
function getD(){
...
(function fixParent(){
  try{
    ...
    s.textContent='.main .block-container{padding:0!important;margin:0!important}.main{padding:0!important}[data-testid="stVerticalBlock"]{gap:0!important}header,[data-testid="stToolbar"]{display:none!important}iframe{margin:0!important}.stMainBlockContainer{padding:0!important}footer{display:none!important}[data-testid="stBottom"]{display:none!important}';
